...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, group=1):
        ...
        self.deconv2 = nn.ConvTranspose2d(in_channels // 4, in_channels // 4, 3, stride=2, padding=1, output_padding=1, groups=group)
...
class Hourglass(nn.Module):
    ...
    def _hourglass_forward(self, n, x):
        ...
        if n > 1:
            low2_1, low2_2 = self._hourglass_forward(n - 1, low1)
        else:
...
class MultiTaskStackedHourglassSelfAttention(nn.Module):
    def __init__(self, task1_classes=2, task2_classes=37, block=ResidualBlock, in_channels=3, num_stacks=2, num_blocks=1, hg_num_blocks=3, depth=3):
        super(MultiTaskStackedHourglassSelfAttention, self).__init__()
        ...
        self.relu = nn.ReLU(inplace=True)
        self.self_attention = SelfAttentionBlock(self.inplanes)
        ...
        for i in range(num_stacks):
            hg.append(Hourglass(block, hg_num_blocks, self.num_feats, depth))
        ...
        # Final Classifier
        self.decoder1 = Decoder(self.num_feats, self.inplanes)
        ...
        # Final Classifier
        self.angle_decoder1 = Decoder(self.num_feats, self.inplanes)
